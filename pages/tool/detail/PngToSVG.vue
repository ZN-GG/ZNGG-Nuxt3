...
// import * as wasm from "~/utils/wasm/pkg/png_to_svg_wasm";
const wasm = (await import("~/utils/wasm/png-to-svg-wasm"))
...
async function selectFile(e) {
    if (e.target.files.length > 0) {
        ...
        reader.onloadend = async (evt: any) => {
            ...
            let data = await wasm.get_svg(u8buffer);
            console.log(data);
            const result = optimize(data, {
                multipass: true,
                plugins: ['preset-default', 'reusePaths'],
                js2svg: {
                    indent: 2,
                    pretty: true,
                },
